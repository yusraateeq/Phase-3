...
```text
Generate a step-by-step implementation plan that is strictly based on the approved specifications.
Do not introduce new features or assumptions beyond what is defined in the spec.

The plan must be organized into clear execution phases and remain implementation-aware
without writing actual code.

====================================
PHASE 1: Specification Toolkit Setup
====================================

- Initialize and configure `.spec-kit/config.yaml`
- Prepare the foundational specification documents:
  - overview.md
  - architecture.md
...
- Design and document the database schema using SQLModel
- Establish connection to Neon PostgreSQL
- Add JWT validation logic at the middleware/dependency level
- Ensure authenticated user identity is consistently extracted and enforced

====================================
PHASE 3: API Layer Development
====================================

- Build all REST endpoints required for task management
- Ensure every request is scoped to the authenticated user
- Implement request validation and standardized error handling

====================================
PHASE 4: Frontend Application Layer
====================================

- Set up Better Auth on the frontend with JWT support
- Create an authenticated API client for backend communication
- Develop task CRUD user interfaces using shadcn/ui components
...
- Confirm strict user data isolation across all layers
- Validate rejection of unauthorized or cross-user access
- Conduct manual end-to-end verification against acceptance scenarios
